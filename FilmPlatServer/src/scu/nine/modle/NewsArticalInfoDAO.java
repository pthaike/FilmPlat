 ...
 * 
 * @see scu.nine.modle.NewsArticalInfo
	...
	public NewsArticalInfo findById(java.lang.Integer id) {
		...
		try {
			NewsArticalInfo instance = (NewsArticalInfo) getSession().get(
					"scu.nine.modle.NewsArticalInfo", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(NewsArticalInfo instance) {
		...
		try {
			List results = getSession().createCriteria(
					"scu.nine.modle.NewsArticalInfo").add(
...
//Ìí¼Ó·½·¨
	...
	public List findAticleByKeyWord(String keyword){
		try
		{
			String hql="from NewsArticalInfo where 1=1 ";
			
			if(!"".equals(keyword) && keyword!=null)
			{
				hql+=" and newsTitle like '%"+keyword+"%'";
			}
